...
export const createComponent = async (req, res) => {
	...
	if (!name?.trim() || !category?.trim()) {
		return res.status(400).json({ error: 'Required fields: name and category.' });
	}
...
export const updateComponent = async (req, res) => {
	...
	if (!name || !category || !id) {
		return res.status(400).json({ error: 'Required fields: name, category, and id.' });
	}
...
export const updateComponentResources = async (req, res) => {
	...
	try {
		const { statusUpdated, linksUpdated } = await ComponentService.updateResources(id, req.body);
...
export const deleteComponent = async (req, res) => {
	...
	try {
		...
		if (error.message === 'Component not found.' || error.message === 'Component not found or could not be erased.') {
